...
*TROIA Platform also supports restful web services for 3rd party applications to access business applications/database over TROIA codes. This section aims to introduce TROIA Rest Web Services infrastructure for 3rd party application developers.*

Introduction
------------

Very similar to SOAP Web Services mentioned in previous sections, Restful Web Services also allows allows programmers define web services/methods using TROIA language to serve 3rd party applications. Simply, a troia web service is defining a class method as web service and web service infrastructure serves this service using a standard wsdl interface.

Before the details of wsdl and web servie infrastructure, you must know that TROIA platform has two versions of restful web services. The first one is totally "stateless" second one is "statefull".

In both options, the system logs in a user account in the background and creates a new session. Therefore, a valid user license is required to log in.


Statefull Rest Service
======================

In stateful service, the process involves logging in, making a service call, and logging out. These calls must be made separately and sequentially.

::
	http://…/rest/statefull/v1/login
	http://…/rest/statefull/v1/callService/TROIA_SERVICE_NAME
	http://…/rest/statefull/v1/logout
