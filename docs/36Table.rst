...
::	
	...
	SELECT 1 AS ORD, USERNAME, 'sval' AS PASSW, CREATEDBY
		FROM IASUSERS
		WHERE CREATEDBY = 'BTAN'
		ORDERBY USERNAME
		INTO SOURCE;

	SELECT 1 AS ORD, USERNAME, 'destval' AS PASSW,  CREATEDBY
		FROM IASUSERS
		WHERE 1 = 2
		INTO TMPTABLE;
	...
	MERGETABLE SOURCE INTO TMPTABLE CRITERIA COLUMNS PASSW, INSERTED VALUES 'sval', 1;
	APPEND ROW TO TMPTABLE; /* as separator */
	MERGETABLE SOURCE INTO TMPTABLE CRITERIA INDEXED INDEX 'INDX' VALUES 'sval', 0;
